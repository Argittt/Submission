...
bike_sharing = pd.read_csv(r"D:\VisualCode\Bangkit\bike_sharing.csv")
...
with st.sidebar:
    ...
    if 'dteday' in bike_sharing.columns:
        min_date = pd.to_datetime(bike_sharing['dteday']).min().date()
        max_date = pd.to_datetime(bike_sharing['dteday']).max().date()
        ...
        filtered_df = bike_sharing[(bike_sharing['dteday'] >= str(start_date)) & (bike_sharing['dteday'] <= str(end_date))]
    else:
